...
public class SqlMapAccountDao extends SqlMapClientDaoSupport implements AccountDao {
  ...
  public void updateAccount(Account account) throws DataAccessException {
    getSqlMapClientTemplate().update("Account.updateAccount", account, 1);
    getSqlMapClientTemplate().update("Account.updateProfile", account, 1);
    if (account.getPassword() != null && account.getPassword().length() > 0) {
      getSqlMapClientTemplate().update("Account.updateSignon", account, 1);
    }
  }
 
	public List getUsernameList() throws DataAccessException {
		return getSqlMapClientTemplate().queryForList("Account.getUsernameList", null);
	}
