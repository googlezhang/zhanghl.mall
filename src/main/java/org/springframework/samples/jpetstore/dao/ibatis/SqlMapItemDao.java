...
public class SqlMapItemDao extends SqlMapClientDaoSupport implements ItemDao {
  ...
  public void updateQuantity(Order order) throws DataAccessException {
    for (int i = 0; i < order.getLineItems().size(); i++) {
      ...
      getSqlMapClientTemplate().update("Item.updateInventoryQuantity", param, 1);
    }
  }

  public boolean isItemInStock(String itemId) throws DataAccessException {
    Integer i = (Integer) getSqlMapClientTemplate().queryForObject("Item.getInventoryQuantity", itemId);
    return (i != null && i.intValue() > 0);
  }

  public List<?> getItemListByProduct(String productId) throws DataAccessException {
    return getSqlMapClientTemplate().queryForList("Item.getItemListByProduct", productId);
  }

  public Item getItem(String itemId) throws DataAccessException {
    Item item = (Item) getSqlMapClientTemplate().queryForObject("Item.getItem", itemId);
		if (item != null) {
			Integer qty = (Integer) getSqlMapClientTemplate().queryForObject("Item.getInventoryQuantity", itemId);
