...
public class SqlMapOrderDao extends SqlMapClientDaoSupport implements OrderDao {
  ...
  private SqlMapSequenceDao sequenceDao;
    ...
    return getSqlMapClientTemplate().queryForList("Order.getOrdersByUsername", username);
  }
  ...
  public Order getOrder(int orderId) throws DataAccessException {
    ...
    Order order = (Order) getSqlMapClientTemplate().queryForObject("getOrder", parameterObject);
		if (order != null) {
    	order.setLineItems(getSqlMapClientTemplate().queryForList("LineItem.getLineItemsByOrderId", new Integer(order.getOrderId())));
		}
    return order;
  }
  ...
  public void insertOrder(Order order) throws DataAccessException {
		...
		getSqlMapClientTemplate().insert("Order.insertOrderStatus", order);
    for (int i = 0; i < order.getLineItems().size(); i++) {
      LineItem lineItem = (LineItem) order.getLineItems().get(i);
      lineItem.setOrderId(order.getOrderId());
      getSqlMapClientTemplate().insert("LineItem.insertLineItem", lineItem);
    }
  }
